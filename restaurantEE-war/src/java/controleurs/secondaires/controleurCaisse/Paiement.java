 ...
 * @author cdi205
 */
public class Paiement implements Serializable, ControleurInterface {
    ...
    public String executer(HttpServletRequest request, HttpServletResponse response) {
        ...
        gestionReglement.create(montant, request.getParameter("origine"), fw.getFacturePersiste());
        fw.getFacturePersiste().getReglements().add(gestionReglement.getReglement());
        if(fw.getPrixTotalHT() > 0){
            request.setAttribute("prix", fw.getPrixTotalHT());
            request.setAttribute("typereg", gestionReglement.getTypeReg());
            System.out.println(gestionReglement.getTypeReg());
